...
class ProductAddon extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, siebel_name, group_id, name_en, name_ar', 'required'),
			array('group_id, enabled', 'numerical', 'integerOnly'=>true),
			array('name, siebel_name, name_en, name_ar', 'length', 'max'=>250),
			array('type', 'length', 'max'=>12),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('product_addon_id, name, siebel_name, group_id, type, name_en, name_ar, enabled', 'safe', 'on'=>'search'),
		);
	}
	...
	{
		return array(
			'product_addon_id' => 'Product Addon',
			'name' => 'Name',
